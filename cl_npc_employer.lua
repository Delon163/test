...
local function OpenJobPanel(job)
	...
	base_menu["selectBtn"].DoClick = function()
		...
		if not IsBoughtJob(job) and IsJobNotRequired(job) then
			local darkenPanel -- Объявляем переменную в области видимости файла

			local function DarkenBackground()
			    darkenPanel = vgui.Create("DPanel")
			    darkenPanel:SetSize(ScrW(), ScrH()) -- Задаем размеры панели, чтобы она занимала весь экран
			    darkenPanel:SetPos(0, 0) -- Размещаем панель в левом верхнем углу
			    darkenPanel.Paint = function() -- Функция рисования панели
			        surface.SetDrawColor(0, 0, 0, 200) -- Устанавливаем черный цвет с прозрачностью 200
			        surface.DrawRect(0, 0, ScrW(), ScrH()) -- Рисуем прямоугольник, заполняющий весь экран
			    end
			end

			-- Код для создания меню подтверждения
			local confirmMenu = vgui.Create("DFrame")
			confirmMenu:SetSize(300, 80) -- Устанавливаем размеры меню
			confirmMenu:Center() -- Размещаем меню по центру экрана
			confirmMenu:SetTitle("Подтверждение покупки") -- Устанавливаем заголовок
			confirmMenu:ShowCloseButton(false) -- Отключаем кнопку закрытия
			confirmMenu:SetMouseInputEnabled(true) -- Включаем обработку событий мыши для меню

			local confirmLabel = vgui.Create("DLabel", confirmMenu)
			confirmLabel:SetText("Вы уверены, что хотите купить эту профессию?")
			confirmLabel:Dock(TOP) -- Размещаем метку вверху меню
			confirmLabel:SetContentAlignment(5) -- Выравниваем текст по центру
			confirmLabel:SizeToContents()

			local buttonPanel = vgui.Create("DPanel", confirmMenu)
			buttonPanel:Dock(BOTTOM) -- Размещаем панель с кнопками внизу меню
			buttonPanel:SetTall(30) -- Устанавливаем высоту панели
			buttonPanel:DockMargin(0, 0, 0, 0) -- Устанавливаем отступы для панели кнопок
			buttonPanel:SetBackgroundColor(Color(0, 0, 0, 0))

			local confirmButton = vgui.Create("DButton", buttonPanel)
			confirmButton:SetText("Да")
			confirmButton:Dock(LEFT) -- Размещаем кнопку "Да" слева
			confirmButton:SetWide(100) -- Устанавливаем ширину кнопки
			confirmButton.DoClick = function()
			    net.Start("ENPC.BuyJob")
			    net.WriteInt(job.team, 16)
			    net.SendToServer()
			    confirmMenu:Close() -- Закрываем меню подтверждения после нажатия кнопки "Да"
			    if darkenPanel then
			        darkenPanel:Remove() -- Удаляем затемненный фон
			    end
			end

			local cancelButton = vgui.Create("DButton", buttonPanel)
			cancelButton:SetText("Отмена")
			cancelButton:Dock(RIGHT) -- Размещаем кнопку "Отмена" справа
			cancelButton:SetWide(100) -- Устанавливаем ширину кнопки
			cancelButton:SetMouseInputEnabled(true) -- Включаем обработку событий мыши для этой кнопки
			cancelButton:DockMargin(0, 0, 0, 0) -- Устанавливаем отступы для кнопки "Отмена"
			cancelButton.DoClick = function()
			    confirmMenu:Close() -- Закрываем меню подтверждения после нажатия кнопки "Отмена"
			    if darkenPanel then
			        darkenPanel:Remove() -- Удаляем затемненный фон
			    end
			end
			...
			DarkenBackground()
			confirmMenu:MakePopup() -- Сделаем меню подтверждения модальным



			-- Остальной код после закрытия меню подтверждения
			if type(job.model) == "table" then
